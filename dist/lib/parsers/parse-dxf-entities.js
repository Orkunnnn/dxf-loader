 ...
 */
export function parseEntities(pairs) {
    const entities = [];
 ...
 */
export function parseEntitiesWithVertices(pairs) {
    const entities = [];
    let i = 0;
...
function parse3DFace(pairs) {
    ...
    const vertices = points
        .filter((p) => p.length > 0)
        .map((p) => (p[2] !== undefined ? [p[0], p[1], p[2]] : [p[0], p[1]]));
    return {
        ...base,
        type: '3DFACE',
        vertices: vertices
...
//# sourceMappingURL=parse-dxf-entities.js.map