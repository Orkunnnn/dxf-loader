...
export function convertHatch(entity, circleSegments, include3D) {
    const polygons = [];
...
function convertBoundaryPath(path, circleSegments, include3D) {
    if (path.type === 'polyline' && path.vertices) {
        return path.vertices.map((v) => include3D && v.z !== undefined ? [v.x, v.y, v.z] : [v.x, v.y]);
    }
    ...
    for (const edge of edges) {
        switch (edge.type) {
            ...
            case 'ellipse': {
                ...
                const majorLength = Math.sqrt(edge.majorAxisEndPoint[0] ** 2 + edge.majorAxisEndPoint[1] ** 2);
...
//# sourceMappingURL=convert-hatch.js.map