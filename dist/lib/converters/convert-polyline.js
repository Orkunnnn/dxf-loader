...
export function convertPolyline(entity, circleSegments, include3D) {
    const vertices = entity.vertices;
    const coords = [];
    for (let i = 0; i < vertices.length; i++) {
        const vertex = vertices[i];
        const coord = include3D && vertex.z !== undefined ? [vertex.x, vertex.y, vertex.z] : [vertex.x, vertex.y];
        coords.push(coord);
...
//# sourceMappingURL=convert-polyline.js.map